...
export default function Header({ marginLeft }: { marginLeft: number }) {
  ...
  return (
    <div
      className={`${
        marginLeft > 150 && "hidden"
      } h-max w-max mx-auto my-auto md:my-0 text-white z-[1500] select-none py-6 px-3 md:bg-black`}
    >
      <div className="flex flex-col items-center absolute left-[50%] -translate-x-[50%] lg:-translate-x-[50%]  top-[50%] -translate-y-[50%] lg:flex-row  lg:justify-between w-full  lg:fixed  lg:w-max">
        <div
          style={{
            transform: `translateX(${
              width > 1024 && marginLeft > 25 ? -25 - 0.8 * marginLeft : "-50"
            }%)`,
          }}
